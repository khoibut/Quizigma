...
function EditQuiz( {openFunction} ) {
    ...
    return (
        <>
            ...
            <div class="flex flex-col w-full bg-blue-200 m-2 border-slate-500 rounded-lg md:p-10 lg:w-4/5 h-fit">
                <div class="self-center font-bold sm:text-2xl m-3">EDIT QUIZIGMA</div>
                <div class="gap-2 lg:grid lg:grid-cols-2" >
                    <label class="flex justify-center justify-self-center w-2/3 items-center aspect-square bg-sky-200 shadow-md border border-blue-300 rounded-xl lg:w-4/5 bg-center bg-cover" style={(image != '' ? {backgroundImage:`url(${image})`} :{ backgroundImage: "none"})}>
                        <button class="w-fit bg-red-400 rounded-full py-3 px-10 text-white hover:bg-rose-500 hover:scale-105 hover:opacity-100 transition-all" style={(image == '' ? {display: "block"} :{display: 'none'})} onClick={() => {setAddImage(true)}}>ADD IMAGE</button>
                    ...
                    <div class="col-span-2 flex justify-end mt-5 justify-self-center lg:w-full" type="submit" value="CREATE"><button class="w-full sm:w-fit bg-red-400 rounded-full py-2 px-16 text-white hover:bg-rose-500 hover:scale-105 transition-all">SAVE</button></div>
