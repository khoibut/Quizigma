...
function SignIn() {
    ...
    function login() {
        ...
        if(flag) {
            axios.get('https://quizigmaapi.onrender.com/api/v1/acc/auth', user)
            .then(function(response) {
                alert("You are in")
            })
            .catch(function(error) {
                console.log(error.data)
                if(error.response.data.error === "Invalid email" || error.response.data.error === "Invalid password") {
        ...
        <>
            ...
            <div class="flex justify-center items-center flex-col bg-blue-300 w-full lg:w-2/3 xl:w-1/2 inset-0 m-auto rounded-t-lg mt-10">
                ...
                <div class="flex flex-col w-3/5 lg:w-1/2 m-2">
                    ...
                    <input ref={(current) => (username = current)} class="rounded-md p-3 outline-none hover:outline-blue-950 hover:outline-offset-2" type="text" id="username" name="username" placeholder="Enter your username" />

                    <label class="mt-2 text-xl" for="email">Email</label>
                    <input ref={(current) => (email= current)} class="rounded-md p-3 outline-none hover:outline-blue-950 hover:outline-offset-2" type="text" id="email" name="email" placeholder="Enter your email" />
                    ...
                    <input ref={(current) => (password= current)} class="rounded-md p-3 outline-none hover:outline-blue-950 hover:outline-offset-2" type="text" id="password" name="password" placeholder="Enter your password" />
